...
//---- Migration $.browser remove problem soultion
(function () {
    var matched, browser;
	...
	$.fn.ajaxUploadPrompt = function( settings ) {
		...
		this.each(function() {
			if (needOverlay) {
	            ...
	            var pp = $('<input type="hidden" name="tempID" value="' + id + '" />').appendTo(form);
				...
				d.change(function() {
					...
					iframe.bind('load', function() {
                        ...
                        try
                        {
						    ...
						    if (origSettings.success) origSettings.success.call(origSettings, data);
                        }
                        catch(ex)
                        {
                            origSettings.tempID = $(this).attr("id");
                            origSettings.crossDomainException.call(origSettings, {});
                        }
					...
					form.submit(function(event) {
						origSettings.beforeSend.call(origSettings, {event:event});
						origSettings.onprogress.call(origSettings, {event:event});
					});
					form.submit();
